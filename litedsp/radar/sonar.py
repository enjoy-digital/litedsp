...
class LiteDSPTVG(LiteXModule):
    ...
    spreading). Latency 6.
    """
    def __init__(self, n_range_bins=1024, data_width=16, gain_frac=8, max_gain_log2=8, with_csr=True):
        check(n_range_bins >= 2, "expected n_range_bins >= 2")
        check(1 <= gain_frac <= 12 and 1 <= max_gain_log2 <= 12, "expected 1 <= gain_frac, max_gain_log2 <= 12")
        ...
        self.exp2  = LiteDSPExp2(in_width=GW, frac_bits=GF, out_frac=OF, out_width=OF + max_gain_log2 + 1, with_csr=False)
        ...
        self.sync += If(xfer,
            If(gs.first, r.eq(1 % N)).Elif(r == N - 1, r.eq(N - 1)).Else(r.eq(r + 1)),   # Holds at N-1.
        )
        ...
        self.comb += [self.source.first.eq(self.tags.source.first), self.source.last.eq(self.tags.source.last)]
        self.sync += [
            If(oadv,
            ...
            If(self.clear, self.saturated.eq(0)).Elif(oadv & both & ~self.bypass & (ovi | ovq), self.saturated.eq(1)),
        ]
    ...
    def add_csr(self):
        GW = len(self.g0)
        self._g0    = CSRStorage(GW, name="g0",    description=f"Log2 gain at bin 0 (signed Q.{self.gain_frac}).")
        ...
        self.comb += [
            self.g0.eq(self._g0.storage), self.k_log.eq(self._k_log.storage), self.k_lin.eq(self._k_lin.storage),
            ...
            self._config.fields.gain_frac.eq(self.gain_frac), self._config.fields.max_gain_log2.eq(self.max_gain_log2),
