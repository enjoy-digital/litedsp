...
class LiteDSPKalmanTracker(_LiteDSPTracker):
    ...
    """
    def __init__(self, n_tracks=4, index_width=12, frac_bits=4, velocity_frac=8, cov_frac=8, cov_width=24,
        data_width=17, with_csr=True, with_irq=False):
        check(4 <= cov_frac <= 12 and cov_frac < cov_width <= 32, "expected 4 <= cov_frac <= 12 < cov_width <= 32")
        self.cov_frac  = cov_frac
        self.cov_width = cov_width
        _LiteDSPTracker.__init__(self, n_tracks=n_tracks, index_width=index_width, frac_bits=frac_bits,
            velocity_frac=velocity_frac, gain_frac=cov_frac, data_width=data_width, with_csr=with_csr,
            with_irq=with_irq)
    ...
    def _add_controls(self):
        ...
        self._cov = {a: {n: [Signal(CW, name=f"{n}_{a}{k}") for k in range(T)] for n in ("P11", "P12", "P22")}
                     for a in ("r", "d")}
    ...
    def _add_update_states(self, fsm, c):
        ...
        pred_sums = {}
        for a, (meas, pred) in (("r", (A["meas_r"], A["pred_r"])), ("d", (A["meas_d"], A["pred_d"]))):
            ...
            self.comb += [
                ...
                s11.eq(covA[a]["P11"][idx] + (covA[a]["P12"][idx] << 1) + covA[a]["P22"][idx] + (self.q >> 2)),
                s12.eq(covA[a]["P12"][idx] + covA[a]["P22"][idx] + (self.q >> 1)),
                s22.eq(covA[a]["P22"][idx] + self.q),
            ]
        sat_any = Signal()
        self.comb += sat_any.eq(reduce_or([pred_sums[a][i] > (1 << CW) - 1 for a in ("r", "d") for i in range(3)]))
        ...
        for a, (V, pred) in (("r", (A["V_r"], A["pred_r"])), ("d", (A["V_d"], A["pred_d"]))):
            ...
            c11, c12, c22 = [Signal((CW + KW + 3, True), name=f"c{n}_{a}") for n in ("11", "12", "22")]
            ...
            self.comb += [
                ...
                n11.eq(rounded(c11, CF)), n12.eq(rounded(c12, CF)), n22.eq(p22p[a] - rounded(c22, CF)),
            ]
            newc[a] = tuple(Signal(CW, name=f"newc_{a}{i}") for i in range(3))
            for dst, src in zip(newc[a], (n11, n12, n22)):
                self.comb += dst.eq(Mux(src < 0, 0, Mux(src > (1 << CW) - 1, (1 << CW) - 1, src[:CW])))
        ...
        fsm.act("KLOAD",
            NextValue(dstep, 0),
            *[NextValue(dv1[a], Cat(Replicate(C(0, 1), CF), p11p[a])) for a in ("r", "d")],   # p11p << CF.
            *[NextValue(dv2[a], Cat(Replicate(C(0, 1), CF), p12p[a])) for a in ("r", "d")],
            *[NextValue(rem1[a], 0) for a in ("r", "d")], *[NextValue(rem2[a], 0) for a in ("r", "d")],
            ...
            *[If(idx == k,
                If(state[k] != TRACK_FREE,
                    If(assigned[k],
                        ...
                        *[NextValue(cov[a][n][k], newc[a][i]) for a in ("r", "d") for i, n in enumerate(("P11", "P12", "P22"))],
                        ...
                        *[NextValue(cov[a][n][k], (p11p, p12p, p22p)[i][a]) for a in ("r", "d") for i, n in enumerate(("P11", "P12", "P22"))],
    ...
    def _add_filter_csr(self):
        ...
        self._p_vel0 = CSRStorage(CW, reset=self.p_vel0.reset.value, name="p_vel0", description="Initial velocity variance.")
