...
class LiteDSPCFAR2D(LiteXModule):
    ...
    def __init__(self, n_range_bins=64, n_doppler_bins=16, n_train=(4, 2), n_guard=(1, 1),
        ...
        check(len(n_train) == 2 and len(n_guard) == 2, "expected n_train / n_guard as (range, doppler) pairs")
        ...
        check(N > 2*R and M > 2*C, "expected n_range_bins > 2*(n_train[0]+n_guard[0]) and n_doppler_bins > 2*(n_train[1]+n_guard[1])")
        self.n_range_bins   = N
        ...
        self.sync += If(adv,
            v1.eq(beat), x1.eq(Mux(real0 & (row < N), x0, 0)), real1.eq(real0 & (row < N)), colok1.eq(col_ok),
        ...
        # -----------------------------------------------------------------------------------
        leave_v, gin_v, gout_v, centre_v = [Signal(data_width, name=n) for n in ("leave_v", "gin_v", "gout_v", "centre_v")]
        self.comb += [
        ...
        clear_hs = [s.eq(0) for s in hs_c[1:] + hs_g[1:] + hs_x[1:] + hs_v[1:] + hs_f[1:] + hs_l[1:]]
        self.sync += If(adv,
            v2.eq(v1),
            If(v1,
                *push,
