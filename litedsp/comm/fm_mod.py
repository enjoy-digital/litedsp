...
class _LiteDSPAngleModulator(LiteXModule):
    ...
    def __init__(self, mode, data_width=16, phase_bits=32, lut_depth=1024, with_csr=True):
        ...
        self.sync += If(adv, prod.eq(self.sink.data*dev_s), v1.eq(xfer), f1.eq(self.sink.first), l1.eq(self.sink.last))
        ...
        cos, sin = sincos_rom(self, addr_phase[PB - addr_bits:], adv, DW, lut_depth, quarter_wave=True)
        self.sync += If(adv, self.source.valid.eq(v1), self.source.first.eq(f1), self.source.last.eq(l1))
    ...
    def add_csr(self):
        PB = self.phase_bits
        self._phase_inc = CSRStorage(PB, name="phase_inc", description="Carrier / centre phase increment per sample.")
        self._deviation = CSRStorage(PB, name="deviation",
            description="Phase increment (FM) / phase offset (PM) at full-scale input, in phase-accumulator units.")
        self.comb += [self.phase_inc.eq(self._phase_inc.storage), self.deviation.eq(self._deviation.storage)]
