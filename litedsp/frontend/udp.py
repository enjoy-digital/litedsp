...
``UDPIQStreamer`` sends the I/Q stream as fixed-size UDP packets (``samples_per_packet``
word-packed samples per datagram, first sample in the LSBs); ``UDPIQReceiver`` is the reverse
...
``LiteEthUDPUserPort``, which keeps them simulable without the full stack:
    ...
    self.iq_streamer = UDPIQStreamer(udp.udp, ip_address="192.168.1.100", udp_port=6000)
...
class UDPIQStreamer(LiteXModule):
    ...
    def __init__(self, udp, ip_address, udp_port, data_width=16, word_width=32,
        ...
        self.packetizer  = IQPacketizer(data_width=data_width, word_width=word_width,
...
class UDPIQReceiver(LiteXModule):
    ...
    def __init__(self, udp, udp_port, data_width=16, word_width=32, fifo_depth=64, with_csr=True):
        ...
        self.depacketizer = IQDepacketizer(data_width=data_width, word_width=word_width,
            with_csr=with_csr)
        port = _get_port(udp, udp_port, word_width)
        self.comb += [
