...
SORT9 = [[(0, 1), (2, 3), (4, 5), (6, 7)] if r % 2 == 0 else [(1, 2), (3, 4), (5, 6), (7, 8)]
    for r in range(9)]
...
class LiteDSPRankFilter(LiteXModule):
    ...
    """
    def __init__(self, data_width=8, n_channels=1, rank=4, width=640, max_width=None,
                 border="replicate",
        with_csr=True):
        check(0 <= rank <= 8, "expected 0 <= rank <= 8")
        self.data_width = data_width
        self.n_channels = n_channels
        self.lb = LiteDSPLineBuffer(data_width, n_channels, 3, width, max_width, border,
                                    with_csr=False)
        ...
        tags = [(Signal(name=f"v{k}"), Signal(name=f"f{k}"), Signal(name=f"e{k}"),
                 Signal(name=f"l{k}")) for k in range(n_regs)]
        ...
        self.sync += If(adv, *[t.eq(s) for t, s in zip(tags[0], src)],
                        *[centres[0][c].eq(self.lb.source.w11[c*DW:(c + 1)*DW])
        ...
        for c in range(n_channels):
            ...
            for stage, pairs in enumerate(SORT9):
                new = list(vals)
                for a, b in pairs:
                    lo, hi = Signal(DW, name=f"lo{c}_{stage}_{a}"), Signal(
                        DW, name=f"hi{c}_{stage}_{b}")
                    self.comb += [lo.eq(Mux(vals[a] < vals[b], vals[a], vals[b])),
                                  hi.eq(Mux(vals[a] < vals[b], vals[b], vals[a]))]
                    new[a], new[b] = lo, hi
        ...
        vN, fN, eN, lN = tags[-1]
        self.sync += If(adv,
            self.source.valid.eq(vN), self.source.first.eq(fN), self.source.eol.eq(eN),
            self.source.last.eq(lN),
            *[getattr(self.source, f).eq(Mux(self.bypass, centres[-1][c], Array(sorted_out[c])[
                self.rank])) for c, f in enumerate(fields)],
        )
