...
from litedsp.common       import check, pixel_layout, pixel_fields, add_bypass, add_bypass_csr, bits_for
...
class LiteDSPBoxOverlay(LiteXModule):
    ...
    latency 1.
    """
    def __init__(self, data_width=8, n_channels=3, n_boxes=4, thickness=1, boxes=None, coord_bits=12, with_csr=True):
        ...
        self.box_x0, self.box_y0, self.box_x1, self.box_y1 = [Signal(coord_bits, name=n) for n in ("box_x0", "box_y0", "box_x1", "box_y1")]
        ...
        for k, (x0, y0, x1, y1, color, en) in enumerate(boxes):
            ...
            init["x0"][k], init["y0"][k], init["x1"][k], init["y1"][k] = int(x0), int(y0), int(x1), int(y1)
        ...
        self.sync += If(self.box_we,
            *[If(self.box_index == k,
                sh["x0"][k].eq(self.box_x0), sh["y0"][k].eq(self.box_y0), sh["x1"][k].eq(self.box_x1), sh["y1"][k].eq(self.box_y1),
        ...
        self.sync += [
            ...
            If(do_commit, *[ac[n][k].eq(sh[n][k]) for n in ac for k in range(N)], self.commit_pending.eq(0)),
        ]
        ...
        for k in reversed(range(N)):                                    # Lowest index wins (last assignment).
            x0, y0, x1, y1 = [Signal((coord_bits + 1, True), name=f"s{n}{k}") for n in ("x0", "y0", "x1", "y1")]
            self.comb += [x0.eq(eff["x0"][k]), y0.eq(eff["y0"][k]), x1.eq(eff["x1"][k]), y1.eq(eff["y1"][k])]
            inside = Signal()
            edge   = Signal()
            self.comb += [
                inside.eq((col >= x0) & (col <= x1) & (row >= y0) & (row <= y1)),
                edge.eq(inside & ((col - x0 < t) | (x1 - col < t) | (row - y0 < t) | (y1 - row < t))),
        ...
        self.sync += If(adv,
            ...
            *[getattr(self.source, f).eq(Mux(hit, color[c*data_width:(c + 1)*data_width], getattr(self.sink, f))) for c, f in enumerate(fields)],
        )
        add_bypass(self)
    ...
    def add_csr(self):
        ...
        self._box_index = CSRStorage(len(self.box_index), name="box_index", description="Shadow box to write.")
        ...
        self.comb += [
            ...
            self.box_color.eq(self._box_color.fields.color), self.box_enable.eq(self._box_color.fields.enable),
            self.box_we.eq(self._box_color.re),
            self.commit.eq(self._control.fields.commit), self.thickness.eq(self._control.fields.thickness),
