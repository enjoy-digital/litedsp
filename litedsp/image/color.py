...
class LiteDSPColorMatrix(LiteXModule):
    ...
    ``n_out == 3``. Latency 3.
    """
    def __init__(self, data_width=8, n_out=3, coefficients=None, in_offsets=(0, 0, 0), out_offsets=(0, 0, 0),
        ...
        if coefficients is None:
            coefficients = color_preset("identity", data_width, coeff_frac)[0] if n_out == 3 else color_preset("rgb_to_gray_601", data_width, coeff_frac)[0]
        ...
        check(len(out_offsets) == n_out and len(in_offsets) == 3, "expected 3 input offsets and n_out output offsets")
        ...
        N = 3*n_out
        OFW = DW + 1
        values = [int(c) for c in coefficients] + [int(v) for v in in_offsets] + [int(v) for v in out_offsets]
        widths = [CW]*N + [OFW]*3 + [OFW]*n_out
        active = [Signal((w, True), reset=v, name=f"act{k}") for k, (w, v) in enumerate(zip(widths, values))]
        shadow = [Signal((w, True), reset=v, name=f"sh{k}") for k, (w, v) in enumerate(zip(widths, values))]
        # Shadow index map: 0..N-1 coefficients, 9..11 input offsets, 12..(12+n_out-1) output offsets.
        idx_of = list(range(N)) + [9, 10, 11] + [12 + k for k in range(n_out)]
        self.sync += [
            If(self.coeff_we,
                *[If(self.coeff_index == idx_of[k], shadow[k].eq(self.coeff_value[:widths[k]])) for k in range(len(shadow))],
        ...
        # S1: centred inputs (registered); S2: products (registered); S3: sums, round, offset, clamp.
        ...
        self.sync += [
            If(adv,
                self.source.valid.eq(v2), self.source.first.eq(f2), self.source.eol.eq(e2), self.source.last.eq(l2),
    ...
    def add_csr(self):
        CW = self.coeff_width
        self._coeff_index = CSRStorage(4, name="coeff_index", description="Shadow index (0..8 matrix, 9..11 input offsets, 12..14 output offsets); auto-increments on a value write.")
        self._coeff_value = CSRStorage(CW, name="coeff_value", description="Writing stores the shadow entry at coeff_index (signed).")
        ...
        self.comb += [
            ...
            self._config.fields.n_out.eq(self.n_out), self._config.fields.coeff_frac.eq(self.coeff_frac),
        ]
        if self.n_out == 3:
            add_bypass_csr(self)
