...
def fm_modulator():
    d = LiteDSPFrequencyModulator(with_csr=False)
...
def line_encoder():
    d = LiteDSPLineEncoder(code="diff_manchester", with_csr=False)
    return d, {d.invert, d.phase_rst} | _eps(d.sink, d.source), 10.0
...
def hamming_encoder():
    ...
    return d, {d.blocks} | _eps(d.sink, d.source), 10.0
...
def convolutional_interleaver():
    ...
    return d, {d.phase_rst, d.bypass} | _eps(d.sink, d.source), 10.0

def convolutional_deinterleaver():
    d = LiteDSPConvolutionalDeinterleaver(with_csr=False)
    return d, {d.phase_rst, d.bypass} | _eps(d.sink, d.source), 10.0

def hdlc_framer():
    d = LiteDSPHDLCFramer(with_csr=False)
    return d, {d.frames} | _eps(d.sink, d.source), 10.0
...
def line_buffer():
    ...
    return d, {d.clear, d.geometry_error, d.line_length} | _eps(d.sink, d.source), 10.0

def line_buffer_5x5_rgb():
    d = LiteDSPLineBuffer(n_channels=3, kernel_size=5, width=640, with_csr=False)
    return d, {d.clear, d.geometry_error, d.line_length} | _eps(d.sink, d.source), 10.0
...
def kernel_2d():
    return _kernel("gaussian3")

def kernel_5x5():
    return _kernel("gaussian5", 5)
...
def pixel_histogram():
    ...
    return d, {d.channel, d.overrun, d.clear, d.frames} | _eps(d.sink, d.source), 10.0

def alpha_blend():
    d = LiteDSPAlphaBlend(with_csr=False)
    return d, {d.alpha} | _eps(d.sink_a, d.sink_b, d.source), 10.0
...
def pixel_unpack():
    d = LiteDSPPixelUnpack(with_csr=False)
    return d, {d.width} | _eps(d.sink, d.source), 10.0
...
REGISTRY = {
    ...
    "range_gate": range_gate, "pulse_generator": pulse_generator, "pixel_pattern": pixel_pattern,
    "fm_modulator": fm_modulator, "am_modulator": am_modulator, "gray_mapper": gray_mapper,
    "line_encoder": line_encoder, "hamming_encoder": hamming_encoder, "hdlc_framer": hdlc_framer,
    "bch_encoder": bch_encoder, "bch_decoder": bch_decoder, "hdlc_deframer": hdlc_deframer,
    "convolutional_interleaver": convolutional_interleaver,
    "convolutional_deinterleaver": convolutional_deinterleaver, "hamming_decoder": hamming_decoder,
    "line_decoder": line_decoder, "gray_demapper": gray_demapper, "ssb_modulator": ssb_modulator,
    "fsk_modulator": fsk_modulator, "pm_modulator": pm_modulator, "pixel_pack": pixel_pack,
    "line_buffer": line_buffer, "line_buffer_5x5_rgb": line_buffer_5x5_rgb,
    "pixel_fifo": pixel_fifo, "pixel_stats": pixel_stats, "pixel_histogram": pixel_histogram,
    "alpha_blend": alpha_blend, "box_overlay": box_overlay, "pixel_lut": pixel_lut,
    "color_matrix": color_matrix, "debayer": debayer, "downscaler": downscaler, "crop": crop,
    "sobel": sobel, "rank_filter": rank_filter, "threshold": threshold, "pixel_gain": pixel_gain,
    "kernel_2d": kernel_2d, "kernel_5x5": kernel_5x5, "kernel_2d_rgb": kernel_2d_rgb,
    "pixel_from_video": pixel_from_video, "pixel_to_video": pixel_to_video,
    "pixel_unpack": pixel_unpack, "pulse_compressor": pulse_compressor,
    "pulse_compressor_mac": pulse_compressor_mac,
    "mti": mti, "corner_turn": corner_turn, "doppler": doppler, "ca_cfar": ca_cfar,
    "cfar_2d": cfar_2d, "os_cfar": os_cfar, "clutter_map": clutter_map,
    "cfar_2d_wide": cfar_2d_wide, "peak_extractor": peak_extractor, "target_list": target_list,
    "alpha_beta_tracker": alpha_beta_tracker, "kalman_tracker": kalman_tracker,
    "beamformer": beamformer, "beamformer_4beams": beamformer_4beams, "monopulse": monopulse,
    "tvg": tvg,
}
