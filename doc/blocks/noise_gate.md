...
| `lookahead` | `0` | int | Delay of the gain-applied signal in frames (0 = none). |
| `preset` | `"gate"` | str | ``"compressor"`` (-20 dB, 4:1, 10/100 ms), ``"limiter"`` (-1 dBFS, instant attack, 50 ms release) or ``"gate"`` (-50 dB, 1:8, 1/100 ms): reset values of the controls. Choices: `compressor`, `limiter`, `gate`. |
