...
`LiteDSPCPInsert` — `litedsp.comm.ofdm` — category `comm`
...
|---|---|---|---|
| `fft_size` | `64` | int | OFDM symbol length N in samples (the IFFT size); sets the symbol buffer (memory) depth. |
| `cp_len` | `16` | int | Cyclic-prefix length in samples, copied from the symbol tail and prepended (0 < cp_len < fft_size). |
