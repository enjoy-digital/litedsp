...
| `architecture` | `"classic"` | str | ``"classic"`` for one-sample delayed LMS, or ``"pipelined"`` for an eight-sample adaptation delay with unchanged filter throughput and two additional output cycles. Choices: `classic`, `pipelined`. |
| `update_pipeline` | `False` | bool | Register the pipelined architecture's completed tap increments before applying them. This makes the adaptation delay nine accepted samples and cuts the multiplier/add tree away from the weight-feedback edge. It is an explicit control-loop trade-off and does not change filter-output latency or peak throughput. |
