...
| `preset` | `"compressor"` | str | ``"compressor"`` (-20 dB, 4:1, 10/100 ms), ``"limiter"`` (-1 dBFS, instant attack, 50 ms release) or ``"gate"`` (-50 dB, 1:8, 1/100 ms): reset values of the controls. Choices: `compressor`, `limiter`, `gate`. |
...
### `threshold` (read-write, 16 bits, reset `0xfcae`)

Threshold in log2 units re full scale (signed Q.8: 256 = 6.02 dB).

### `slope_above` (read-write, 20 bits, reset `0xc000`)
...
### `attack` (read-write, 17 bits, reset `0x88`)
