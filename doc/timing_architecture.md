...
| QPSK receiver IP | 41.7 MHz classic; 109.0 MHz four-sample loop median | NCO LUT, four mixer products, decision detector, PI update, and phase recurrence | accepted-sample delayed loop landed and target-closed |
...
operands, registers all mixer products, and registers the rounded/saturated derotation before the
decision detector. A four-entry observation queue applies error *n* only after three newer input samples have been
...
|---|---:|---:|---:|---:|---:|
| Pipelined, ECP5-85 (three-route median) | 1773 | 851 | 2 | 20 | 109.0 MHz |
| Pipelined, Artix-7 `xc7a200t-3` (three-strategy median) | 950 | 562 | 1 | 12 | 129.7 MHz |
| Pipelined, Artix UltraScale+ `xcau20p-2` (three-strategy median) | 943 | 550 | 1 | 12 | 245.9 MHz |

Relative to the classic complete core, ECP5 moves from 2476 LUT / 612 FF / 0 BRAM / 20 DSP to
1773 / 851 / 2 / 20; the registered NCO boundary lets both tables infer as BRAM. Artix-7 moves
from 885 LUT / 455 FF / 1 BRAM / 12 DSP to 950 / 562 / 1 / 12. Output latency rises from one to
