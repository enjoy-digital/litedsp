...
class TestLineBuffer(unittest.TestCase):
    def _check(self, cap, imgs, K, border, w, h):
        ...
        for k, img in enumerate(imgs):
            ...
            for f in fields:
                self.assertEqual(column(cap[k*n:(k + 1)*n], f).tolist(), ref[f].tolist(),
                                 f"frame {k} {f}")
    ...
    def test_bit_exact(self):
        cases = [(3, "replicate", 1), (3, "mirror", 1), (3, "zero", 1), (5, "mirror", 3),
                 (5, "replicate", 1)]
        for K, border, nc in cases:
            with self.subTest(K=K, border=border, nc=nc):
                imgs = images(2, 16, 12, nc, seed=K + len(border))
                dut  = LiteDSPLineBuffer(n_channels=nc, kernel_size=K, width=16, border=border,
                                         with_csr=False)
                fields = [f"w{i}{j}" for i in range(K) for j in range(K)] + ["eol", "first", "last"]
                cap = run_frames(dut, imgs, 2*16*12, nc, source_fields=fields, sink_throttle=0.2,
    ...
    def test_geometry_and_latency(self):
        ...
        cap = run_frames(dut, imgs, 16*6 + 12*6, 1, source_fields=fields, sink_throttle=0.0,
        ...
        run_frames(dut, images(1, 24, 4, 1, 9), 1, 1, source_fields=fields, sink_throttle=0.0,
                   source_ready_rate=1.0,
            extra=[self._status(dut)])
        ...
        run_frames(dut, images(1, 16, 4, 1, 10), 1, 1, source_fields=fields, sink_throttle=0.0,
                   source_ready_rate=1.0,
            extra=[self._timing(dut)])
        self.assertEqual(self.first_out - self.first_in, dut.latency)
        for kwargs in ({"kernel_size": 4}, {"width": 2}, {"border": "wrap"},
                       {"width": 32, "max_width": 16}):
