...
def framing(w, h, n_frames):
    n = w*h
    return ([int(k % w == w - 1) for k in range(n*n_frames)],
            [int(k % n == 0) for k in range(n*n_frames)],
    ...
    def test_bit_exact(self):
        for w, h, D, nc in ((16, 12, 2, 1), (16, 12, 4, 3), (15, 11, 2, 1)):
            with self.subTest(w=w, h=h, D=D, nc=nc):
                ...
                dut  = LiteDSPDownscaler(n_channels=nc, decimation=D, width=w, height=h,
                                         with_csr=False)
                tw, th = w//D, h//D
                cap = run_frames(dut, imgs, 2*tw*th, nc, sink_throttle=0.2, source_ready_rate=0.7)
                for k, img in enumerate(imgs):
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*tw*th:], tw, th, nc),
                                                   downscaler_model(img, D)), f"frame {k}")
    ...
    def test_roi(self):
        for nc in (1, 3):
            with self.subTest(nc=nc):
                ...
                dut  = LiteDSPCrop(n_channels=nc, x0=3, y0=2, roi_width=8, roi_height=6,
                                   with_csr=False)
                cap  = run_frames(dut, imgs, 2*48, nc, sink_throttle=0.2, source_ready_rate=0.7)
                for k, img in enumerate(imgs):
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*48:], 8, 6, nc),
                                                   crop_model(img, 3, 2, 8, 6)), f"frame {k}")
        ...
        def update():
            for _ in range(30):
                yield
            yield dut.x0.eq(4); yield dut.y0.eq(4); yield dut.roi_width.eq(
                4); yield dut.roi_height.eq(4)
            yield dut.commit.eq(1)
            yield
            yield dut.commit.eq(0)
        cap = run_frames(dut, imgs, 192 + 16, 1, sink_throttle=0.0, source_ready_rate=1.0,
                         extra=[update()])
        self.assertTrue(np.array_equal(beats_to_image(cap, 16, 12), imgs[0]))
        self.assertTrue(
            np.array_equal(beats_to_image(cap[192:], 4, 4), crop_model(imgs[1], 4, 4, 4, 4)))
        dut = LiteDSPCrop(n_channels=1, x0=10, y0=0, roi_width=8, roi_height=4, with_csr=False)
        run_frames(dut, imgs[:1], 24, 1, sink_throttle=0.0, source_ready_rate=1.0,
                   extra=[self._status(dut, 250)])
