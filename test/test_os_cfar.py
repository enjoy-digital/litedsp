...
class TestOSCFAR(unittest.TestCase):
    ...
    def test_bit_exact(self):
        ...
        for rank in (0, 5, 7):
            with self.subTest(rank=rank):
                ...
                dut.rank.reset = rank
                cap = run_stream(dut, beats, 2*64, ["data", "first", "last"], FIELDS, sink_throttle=0.2, source_ready_rate=0.7)
    ...
    def test_interferer_masking(self):
        ...
        cap = run_stream(os_, beats, 64, ["data", "first", "last"], FIELDS, sink_throttle=0.0, source_ready_rate=1.0)
        ...
        cap = run_stream(ca, beats, 64, ["data", "first", "last"], FIELDS, sink_throttle=0.0, source_ready_rate=1.0)
        self.assertEqual(sorted(np.flatnonzero(column(cap, "detect")).tolist()), [30])
        self.assertEqual(column(cap, "detect").tolist(), list(ca_cfar_model(x, first, last, 4, 1, alpha=4 << 8)[2]))
