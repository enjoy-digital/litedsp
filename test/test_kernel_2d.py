...
class TestKernel2D(unittest.TestCase):
    ...
    def test_bit_exact(self):
        for name, nc in (("identity", 1), ("gaussian3", 3), ("sobel_x", 1), ("gaussian5", 1)):
            with self.subTest(preset=name, nc=nc):
                ...
                dut  = LiteDSPKernel2D(n_channels=nc, kernel_size=K, coefficients=coefficients,
                ...
                for k, img in enumerate(imgs):
                    ...
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, nc), ref),
                                    f"frame {k}")
    ...
    def test_edges_constant_saturation(self):
        ...
        y = beats_to_image(
            run_frames(dut, [step], 128, 1, sink_throttle=0.0, source_ready_rate=1.0), 16, 8)
        ...
        y = beats_to_image(run_frames(dut, [flat], 96, 1, sink_throttle=0.0, source_ready_rate=1.0),
                           16, 6)
        ...
        self.sat = None
        # 5 * 240 - 3 * 240 ...
        run_frames(dut, [step*12], 128, 1, sink_throttle=0.0, source_ready_rate=1.0,
                   extra=[self._sat(dut)])
    ...
    def test_commit_and_bypass(self):
        ...
        cap = run_frames(dut, imgs, 2*128, 1, sink_throttle=0.0, source_ready_rate=1.0,
                         extra=[loader()])
        ...
        for kwargs in ({"coefficients": [1]*8}, {"shift": 16}, {"coeff_width": 1},
                       {"coefficients": [600]*9}):
