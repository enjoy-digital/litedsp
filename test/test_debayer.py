...
class TestDebayer(unittest.TestCase):
    ...
    def test_bit_exact(self):
        ...
        raws = [np.array([[prng.randint(0, 255) for _ in range(16)] for _ in range(12)]) for _ in range(2)]
        for pattern, border in [(p, "mirror") for p in BAYER_PATTERNS] + [("rggb", "replicate")]:
            with self.subTest(pattern=pattern, border=border):
                ...
                cap = run_frames(dut, raws, 384, 1, source_fields=RGB, sink_throttle=0.2, source_ready_rate=0.7)
                for k, raw in enumerate(raws):
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, 3), debayer_model(raw, pattern, border)), f"frame {k}")
    ...
    # smooth colour gradient comes back with PSNR >= 40 dB (bilinear is exact on linear ramps); a runtime phase flip on a cropped (shifted)
    ...
    def test_reconstruction(self):
        ...
        out = beats_to_image(run_frames(dut, [mosaic(flat, "rggb")], 192, 1, source_fields=RGB, sink_throttle=0.0, source_ready_rate=1.0), 16, 12, 3)
        ...
        out = beats_to_image(run_frames(dut, [mosaic(grad, "rggb")], 384, 1, source_fields=RGB, sink_throttle=0.0, source_ready_rate=1.0), 32, 12, 3)
        ...
        dut.phase.reset = 1
        out = beats_to_image(run_frames(dut, [shifted], 180, 1, source_fields=RGB, sink_throttle=0.0, source_ready_rate=1.0), 15, 12, 3)
