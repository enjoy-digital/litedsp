...
class TestLineCodes(unittest.TestCase):
    ...
    def test_codes(self):
        ...
        for code in CODES:
            with self.subTest(code=code):
                ...
                cap = run_stream(enc, [{"data": b} for b in bits], n_chips, ["data"], ["data"],
    ...
    def test_violations_and_invert(self):
        ...
        cap = run_stream(dec, [{"data": c} for c in chips], 5, ["data"], ["data"],
                         sink_throttle=0.0, source_ready_rate=1.0, extra=[self._status(dec)])
        ...
        self.assertEqual(column(cap, "data").tolist(),
                         line_encode_model([1, 0, 1, 1], "nrzi_m", 1).tolist())
