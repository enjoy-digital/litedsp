...
class TestPixelStats(unittest.TestCase):
    ...
    def test_stats(self):
        ...
        for got, ref in zip(self.seen, refs):
            self.assertEqual(got["sum"], ref["sum"]); self.assertEqual(
                got["min"], ref["min"]); self.assertEqual(got["max"], ref["max"])
            self.assertEqual(got["count"], ref["count"]); self.assertEqual(got["zones"],
                                                                           ref["zones"])
        ...
        self.seen = []
        run_frames(dut, [ramp], 192, 1, sink_throttle=0.0, source_ready_rate=1.0,
                   extra=[self._watch(dut, 300)])
        self.assertEqual((self.seen[0]["min"], self.seen[0]["max"], self.seen[0]["zones"][0]),
                         (0, 48, int(ramp.sum())))
        ...
        def gen():                                                    # Active: outlives the capture
            for _ in range(cycles):                                     # so the last latch is seen.
                if (yield dut.update):
                    ...
                    self.seen.append(dict(sum=(yield dut.sum), min=(yield dut.min), max=(
                        yield dut.max), count=(yield dut.count), zones=zones))
                yield
        return gen()

class TestPixelHistogram(unittest.TestCase):
    # verify-tier: model — three frames (16 x 12 mono, 16 bins; 32 x 24 RGB channel 2, 256 bins)
    # under
    ...
    def test_histograms(self):
        ...
        for nc, ch, bl, w, h in ((1, 0, 4, 16, 12), (3, 2, 8, 32, 24)):
            with self.subTest(nc=nc, bins_log2=bl):
                ...
                dut  = LiteDSPPixelHistogram(n_channels=nc, channel=ch, bins_log2=bl,
                                             with_csr=False)
                cap  = run_frames(dut, imgs, 3*(1 << bl), nc, source_fields=[
                    "data", "first", "last"], sink_throttle=0.2, source_ready_rate=0.7,
                    extra=[self._status(dut)])
                nb = 1 << bl
                for k, img in enumerate(imgs):
                    self.assertEqual(column(cap[k*nb:(k + 1)*nb], "data").tolist(),
                                     histogram_model(img, ch, bl).tolist(), f"frame {k}")
                self.assertEqual(column(cap, "first").tolist(),
                                 [int(k % nb == 0) for k in range(3*nb)])
                self.assertEqual(column(cap, "last").tolist(),
                                 [int(k % nb == nb - 1) for k in range(3*nb)])
        ...
        run_frames(dut, imgs, 8, 1, source_fields=["data"], sink_throttle=0.0,
                   source_ready_rate=0.02, extra=[self._status(dut)])
