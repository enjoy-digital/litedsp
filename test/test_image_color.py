...
class TestPixelLUT(unittest.TestCase):
    ...
    def test_tables(self):
        ...
        for k in range(2):
            self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, 3),
                                           pixel_lut_model(rgb[k], table)))
        ...
        cap = run_frames(dut, frames6, 6*192, 3, sink_throttle=0.0, source_ready_rate=1.0,
                         extra=[loader()])
        self.assertTrue(np.array_equal(beats_to_image(cap[5*192:], 16, 12, 3),
                                       pixel_lut_model(frames6[5], tables)))
        dut = LiteDSPPixelLUT(n_channels=1, gamma=2.2, with_csr=False)
        dut.bypass.reset = 1
        self.assertTrue(
            np.array_equal(beats_to_image(run_frames(dut, imgs[:1], 192, 1), 16, 12), imgs[0]))
...
class TestColorMatrix(unittest.TestCase):
    ...
    def test_bit_exact(self):
        ...
        for name in ("identity", "rgb_to_ycbcr_601", "ycbcr_to_rgb_601", "rgb_to_ycbcr_709",
                     "rgb_to_ycbcr_jpeg", "ycbcr_to_rgb_jpeg", "rgb_to_gray_601"):
            with self.subTest(preset=name):
                c, i, o = color_preset(name)
                n_out = len(c)//3
                dut = LiteDSPColorMatrix(n_out=n_out, coefficients=c, in_offsets=i, out_offsets=o,
                                         with_csr=False)
                cap = run_frames(dut, imgs, 384, 3, source_fields=(
                    ["data"] if n_out == 1 else ["r", "g", "b"]) + ["eol", "first", "last"],
                    sink_throttle=0.2, source_ready_rate=0.7)
                for k in range(2):
                    ref, _ = color_matrix_model(imgs[k], c, i, o)
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, n_out), ref),
                                    f"frame {k}")
    ...
    def test_round_trip_and_commit(self):
        ...
        pix  = np.array([[prng.randint(0, 255) for _ in range(3)] for _ in range(64)]
            + [list(p) for p in itertools.product((0, 255), repeat=3)])
        img  = pix.reshape(8, 9, 3)
        for fwd, inv, tol in (("rgb_to_ycbcr_jpeg", "ycbcr_to_rgb_jpeg", 1),
                              ("rgb_to_ycbcr_601", "ycbcr_to_rgb_601", 2)):
        ...
        for name, expect in (("rgb_to_gray_709", 255), ("rgb_to_ycbcr_601", 235)):
            ...
            dut = LiteDSPColorMatrix(n_out=n_out, coefficients=c, in_offsets=i, out_offsets=o,
                                     with_csr=False)
            cap = run_frames(dut, [white], 32, 3, source_fields=(["data"] if n_out == 1 else ["r",
                "g", "b"]), sink_throttle=0.0, source_ready_rate=1.0)
        ...
        dut = LiteDSPColorMatrix(coefficients=c_id, in_offsets=i_id, out_offsets=o_id,
                                 with_csr=False)
        def loader():
            for _ in range(20):
                yield
        ...
        cap = run_frames(dut, imgs, 256, 3, sink_throttle=0.0, source_ready_rate=1.0,
                         extra=[loader()])
        self.assertTrue(np.array_equal(beats_to_image(cap, 16, 8, 3), imgs[0]))
        self.assertTrue(np.array_equal(beats_to_image(cap[128:], 16, 8, 3),
                                       color_matrix_model(imgs[1], c_y, i_y, o_y)[0]))
