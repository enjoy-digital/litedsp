    ...
    def capture():
        ...
        while len(out) < n_out:
            ...
            if (yield dut.source.valid) and (yield dut.source.ready):
                ...
                out.append(beat)
    run_simulation(dut, [driver(getattr(dut, name), beats, rate) for (name, beats), rate in zip(streams.items(), rates)] + [capture()])
    return out
    ...
    def test_blend(self):
        ...
        for alpha in (128, 0, 256, 77):
            with self.subTest(alpha=alpha):
                ...
                out = simulate_join(dut, {"sink_a": raster_beats(a, 3), "sink_b": raster_beats(b, 3)}, 192,
                    ["r", "g", "b", "eol", "first", "last"], rates=(0.8, 0.6))
                self.assertTrue(np.array_equal(beats_to_image(out, 16, 12, 3), alpha_blend_model(a, b, alpha)))
        ...
        out = simulate_join(dut, {"sink_a": raster_beats(a, 3), "sink_b": raster_beats(b, 3), "sink_alpha": raster_beats(mask, 1)}, 192,
            ["r", "g", "b"], rates=(0.9, 0.7, 0.5))
        self.assertTrue(np.array_equal(beats_to_image(out, 16, 12, 3), alpha_blend_model(a, b, mask)))
...
class TestBoxOverlay(unittest.TestCase):
    ...
    def test_boxes(self):
        boxes = [(2, 1, 9, 6, (255, 0, 0), 1), (6, 4, 14, 10, (0, 255, 0), 1), (12, 8, 20, 15, (0, 0, 255), 1), (0, 0, 3, 3, (9, 9, 9), 0)]
        for nc in (3, 1):
            with self.subTest(nc=nc):
                imgs = images(2, 16, 12, nc, seed=nc)
                bx   = [(x0, y0, x1, y1, c if nc == 3 else c[0], en) for (x0, y0, x1, y1, c, en) in boxes]
                dut  = LiteDSPBoxOverlay(n_channels=nc, n_boxes=4, thickness=2, with_csr=False)
                def loader():
                    for k, (x0, y0, x1, y1, c, en) in enumerate(bx):
                        packed = sum(int(v) << (8*i) for i, v in enumerate(c)) if nc == 3 else int(c)
                        yield dut.box_index.eq(k); yield dut.box_x0.eq(x0); yield dut.box_y0.eq(y0); yield dut.box_x1.eq(x1); yield dut.box_y1.eq(y1)
                        yield dut.box_color.eq(packed); yield dut.box_enable.eq(en); yield dut.box_we.eq(1)
                        yield
                ...
                cap = run_frames(dut, imgs, 384, nc, sink_throttle=0.2, source_ready_rate=0.7, extra=[loader()])
                self.assertTrue(np.array_equal(beats_to_image(cap, 16, 12, nc), imgs[0]))
                self.assertTrue(np.array_equal(beats_to_image(cap[192:], 16, 12, nc), box_overlay_model(imgs[1], bx, 2)))
                self.assertEqual(dut.latency, 1)
        dut = LiteDSPBoxOverlay(with_csr=False)
        dut.bypass.reset = 1
        img = images(1, 16, 12, 3, 4)[0]
        self.assertTrue(np.array_equal(beats_to_image(run_frames(dut, [img], 192, 3), 16, 12, 3), img))
