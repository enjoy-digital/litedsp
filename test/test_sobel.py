...
def images(n, w, h, seed):
    ...
    return [np.array([[prng.randint(0, 255) for _ in range(w)] for _ in range(h)]) for _ in range(n)]
...
class TestSobel(unittest.TestCase):
    ...
    def test_bit_exact(self):
        for mode, with_dir in (("l1", True), ("linf", False), ("approx", False)):
            with self.subTest(mode=mode):
                ...
                cap = run_frames(dut, imgs, 2*192, 1, source_fields=fields, sink_throttle=0.2, source_ready_rate=0.7)
                for k, img in enumerate(imgs):
                    ref = sobel_model(img, mode, 3, "replicate", with_direction=with_dir)
                    mag, direction = ref if with_dir else (ref, None)
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12), mag), f"frame {k}")
                    if with_dir:
                        self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, field="direction"), direction))
    ...
    def test_edges_and_bypass(self):
        ...
        cap = run_frames(dut, [step], 128, 1, source_fields=["data", "direction"], sink_throttle=0.0, source_ready_rate=1.0)
        ...
        dut = LiteDSPSobel(width=16, with_direction=True, with_csr=False)   # (a module simulates once)
        cap = run_frames(dut, [hstep], 128, 1, source_fields=["data", "direction"], sink_throttle=0.0, source_ready_rate=1.0)
        ...
        dut = LiteDSPSobel(width=16, with_direction=True, with_csr=False)
        cap = run_frames(dut, [ramp], 128, 1, source_fields=["data", "direction"], sink_throttle=0.0, source_ready_rate=1.0)
        d = beats_to_image(cap, 16, 8, field="direction")
