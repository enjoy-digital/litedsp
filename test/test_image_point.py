...
class TestRankFilter(unittest.TestCase):
        ...
        for rank, nc in ((0, 1), (4, 1), (8, 1), (4, 3)):
            with self.subTest(rank=rank, nc=nc):
                ...
                for k, img in enumerate(imgs):
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, nc), rank_filter_model(img, rank)), f"frame {k}")
    ...
    def test_morphology(self):
        ...
        self.assertTrue(np.array_equal(beats_to_image(run_frames(dut, [noisy], 192, 1, sink_throttle=0.0, source_ready_rate=1.0), 16, 12), flat))
        square = np.zeros((12, 16), np.int64); square[4:8, 6:10] = 255
        er = beats_to_image(run_frames(LiteDSPRankFilter(rank=0, width=16, with_csr=False), [square], 192, 1, sink_throttle=0.0, source_ready_rate=1.0), 16, 12)
        self.assertEqual(int((er == 255).sum()), 4)
        self.assertTrue(np.all(er[5:7, 7:9] == 255))
        di = beats_to_image(run_frames(LiteDSPRankFilter(rank=8, width=16, with_csr=False), [square], 192, 1, sink_throttle=0.0, source_ready_rate=1.0), 16, 12)
        ...
        def change():
            for _ in range(250):
                yield
            yield dut.rank.eq(8)
        cap = run_frames(dut, [square, square], 384, 1, sink_throttle=0.0, source_ready_rate=1.0, extra=[change()])
    ...
    def test_bit_exact_and_hysteresis(self):
        ...
        for high, low, inv in ((160, 96, 0), (128, None, 0), (200, 100, 1)):
            with self.subTest(high=high, low=low, invert=inv):
                ...
                for k, img in enumerate(imgs):
                    self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12), threshold_model(img, high, low, inv)), f"frame {k}")
                self.assertEqual(dut.latency, 1)
        prng = random.Random(2)
        ramp = np.array([[min(255, 20*x + prng.randint(-15, 15)) for x in range(16)] for _ in range(4)])
        ramp[:, 0] = 0
        dut = LiteDSPThreshold(high=160, low=96, with_csr=False)
        y = beats_to_image(run_frames(dut, [ramp], 64, 1, sink_throttle=0.0, source_ready_rate=1.0), 16, 4)
        for row in y:
...
class TestPixelGain(unittest.TestCase):
    ...
    def test_bit_exact(self):
        ...
        cap = run_frames(dut, imgs, 384, 3, sink_throttle=0.2, source_ready_rate=0.7, extra=[self._sat(dut)])
        for k, img in enumerate(imgs):
            ref, _ = pixel_gain_model(img, gains, offsets)
            self.assertTrue(np.array_equal(beats_to_image(cap[k*192:], 16, 12, 3), ref), f"frame {k}")
        self.assertEqual(dut.latency, 2)
        mono = images(1, 16, 12, 1, 13)
        dut = LiteDSPPixelGain(n_channels=1, with_csr=False)
        self.assertTrue(np.array_equal(beats_to_image(run_frames(dut, mono, 192, 1), 16, 12), mono[0]))
        dut = LiteDSPPixelGain(n_channels=1, with_csr=False)
        dut.gain[0].reset = 512
        run_frames(dut, mono, 192, 1, sink_throttle=0.0, source_ready_rate=1.0, extra=[self._sat(dut)])
